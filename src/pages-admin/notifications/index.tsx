...
type Notification = { id: string; type: string; title: string; content: string; orderId?: string; senderName?: string; senderPhone?: string; statusLabel?: string; targetPath?: string; readAt?: string; createdAt: string }
...
export default function AdminNotifications() {
  ...
  const read = async (row: Notification) => {
    if (!row.readAt) {
      ...
      setData(current => ({ ...current, unreadCount: Math.max(0, current.unreadCount - 1), items: current.items.map(item => item.id === row.id ? { ...item, readAt } : item) }))
      try {
        await adminRequest({ url: `/api/admin/notifications/${row.id}/read`, method: 'PUT' })
      } catch (reason) {
        await Taro.showToast({ title: (reason as Error).message || '标记已读失败', icon: 'none' })
        await load()
        return
      }
    }
    const orderId = row.orderId || row.targetPath?.match(/^\/orders\/([^/]+)/)?.[1]
    if (orderId) await Taro.navigateTo({ url: `/pages-admin/order-detail/index?id=${encodeURIComponent(orderId)}` })
    else if (row.targetPath?.includes('rental')) await Taro.navigateTo({ url: '/pages-admin/inquiries/index?type=rental' })
    else if (row.targetPath?.includes('inquiries')) await Taro.navigateTo({ url: '/pages-admin/inquiries/index?type=monthly' })
  }
