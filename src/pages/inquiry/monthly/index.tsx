...
import { ArrowLeft, MapPin, User, Phone, Building2, Package, Calendar, Repeat } from 'lucide-react-taro'
...
const MonthlyInquiryPage: FC = () => {
  ...
  const isFormValid = senderAddress.trim() && receiverAddress.trim()
  ...
  const handleSubmitAndSelectVehicle = () => {
    ...
    if (preselectedVehicleId) {
      ...
      const params = [
        `senderAddress=${encodeURIComponent(senderAddress)}`,
        `receiverAddress=${encodeURIComponent(receiverAddress)}`,
          ...
          <CardContent className="p-4">
            ...
            <View className="mb-3">
              <View className="flex items-center gap-2 mb-1">
                <MapPin size={14} color="#2088D8" />
                <Text className="block text-sm font-medium text-slate-700">发货地址 *</Text>
              </View>
              <Input placeholder="请输入发货地址" value={senderAddress} onInput={e => setSenderAddress(e.detail.value)} />
            </View>
            <View>
              <View className="flex items-center gap-2 mb-1">
                <MapPin size={14} color="#F59E0B" />
                <Text className="block text-sm font-medium text-slate-700">收货地址 *</Text>
              </View>
              <Input placeholder="请输入收货地址" value={receiverAddress} onInput={e => setReceiverAddress(e.detail.value)} />
            </View>
          </CardContent>
        </Card>
