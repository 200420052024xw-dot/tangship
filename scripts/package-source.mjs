...
const forbidden = tracked.filter(file => /(^|\/)(\.env(\..+)?|node_modules|dist[^/]*|server\/data|server\/backups)(\/|$)|\.(db|sqlite|log|session)$|-(wal|shm)$/.test(file) && !file.endsWith('.env.example'))
